...
describe('DomainResourceInboundEditor source', () => {
  ...
  it('exposes naming-segment toggles instead of free-text prefix/suffix', () => {
    expect(source).toContain('const includeProtocol = ref(true)')
    expect(source).toContain('const includeSecurity = ref(true)')
    expect(source).toContain('const includeFlag = ref(true)')
    expect(source).toContain('v-switch v-model="includeProtocol"')
    expect(source).toContain('v-switch v-model="includeSecurity"')
    expect(source).toContain('v-switch v-model="includeFlag"')
    expect(source).toContain("$t('clusterCenter.domainResources.includeProtocol')")
    expect(source).toContain("$t('clusterCenter.domainResources.includeSecurity')")
    expect(source).toContain("$t('clusterCenter.domainResources.includeFlag')")
    expect(source).toContain("$t('clusterCenter.domainResources.preview')")
    expect(source).toContain('namingPreview')
    expect(source).toContain('countryToFlag')
    expect(source).toContain('securityLabel')
    expect(source).not.toContain('const prefix = ref')
    expect(source).not.toContain('const suffix = ref')
    expect(source).not.toContain("v-model=\"prefix\"")
    expect(source).not.toContain("v-model=\"suffix\"")
  })

  it('sends the segment-toggle flags in the create payload instead of prefix/suffix', () => {
    expect(source).toContain('include_protocol: includeProtocol.value')
    expect(source).toContain('include_security: includeSecurity.value')
    expect(source).toContain('include_flag: includeFlag.value')
    expect(source).not.toContain('prefix: prefix.value')
    expect(source).not.toContain('suffix: suffix.value')
  })
})
