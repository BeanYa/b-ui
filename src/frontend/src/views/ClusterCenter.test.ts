...
describe('ClusterCenter view source', () => {
  ...
  it('marks domain resources as Domain-managed and exposes create/retry status controls in domain detail', () => {
    ...
    expect(source).toContain('const parseDomainResourceJson = (value: string, label: string)')
    expect(source).toContain('const submitDomainInboundResource = async () => {')
    expect(source).toContain('parseDomainResourceJson(domainInboundForm.value.inboundJson')
    expect(source).toContain('await createDomainInboundResource(selectedDomain.value.id, {')
    expect(source).toContain('const submitDomainUserResource = async () => {')
    expect(source).toContain('parseDomainResourceJson(domainUserForm.value.configJson')
