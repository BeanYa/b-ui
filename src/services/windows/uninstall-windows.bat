...
echo Uninstalling B-UI from: %INSTALL_DIR%

REM Stop and remove Windows Service
if exist "%INSTALL_DIR%\b-ui-service.exe" (
    echo Stopping and removing Windows Service...
    net stop %SERVICE_NAME% >nul 2>&1
    cd /d "%INSTALL_DIR%"
    b-ui-service.exe uninstall >nul 2>&1
    if %errorLevel% equ 0 (
...
if exist "%DESKTOP%\B-UI.lnk" (
    del "%DESKTOP%\B-UI.lnk" >nul 2>&1
    echo Desktop shortcut removed
)

REM Remove Start Menu shortcut
echo Removing Start Menu shortcut...
set "START_MENU=%APPDATA%\Microsoft\Windows\Start Menu\Programs\B-UI"
if exist "%START_MENU%" (
...
reg delete "HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Session Manager\Environment" /v BUI_HOME /f >nul 2>&1
...
netsh advfirewall firewall delete rule name="B-UI Panel" >nul 2>&1
netsh advfirewall firewall delete rule name="B-UI Subscription" >nul 2>&1
...
echo ========================================
echo.
echo B-UI has been uninstalled from your system.
echo.
if /i "%keep_data%"=="y" (
