# Contributing to B-UI Frontend

Thank you for your interest in contributing to B-UI Frontend. The frontend now lives directly inside the main `BeanYa/b-ui` repository under `frontend/`, so frontend changes are submitted as part of the main repository rather than through a separate frontend repo.
...
2. **Clone** your fork locally:
   ```bash
git clone https://github.com/YOUR_USERNAME/b-ui.git
cd b-ui/frontend
   ```
3. Add the upstream remote from the repository root (optional, for syncing):
   ```bash
cd ..
git remote add upstream https://github.com/BeanYa/b-ui.git
   ```
