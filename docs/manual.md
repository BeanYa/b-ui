...
```
buihub://hub.example.com/domain?id=example.com&domain_token=xxxx
```
...
域详情页面底部有"活动日志"面板，展示集群相关的操作记录：

- **日志类型**：`INBOUND`（入站）、`OUTBOUND`（出站）、`HUB`（Hub 通信）、`CRON`（定时任务）
- **过滤**：可按域名过滤日志内容
- **容量**：内存环形缓冲区最多保留 2048 条，超出后旧记录自动淘汰
- **查看**：默认显示最近 200 条，支持手动滚动浏览

### 10.7 节点详情与延迟卡片

点击成员节点进入详情页面（ClusterNodeDetail.vue）：

- **基本详情**：节点 ID、名称、地址、Base URL、面板版本、状态
- **延迟卡片**：可折叠面板显示 ping 延迟，带颜色编码（绿色正常、黄色警示、红色超时），支持按目标成员筛选
- **延迟数据来源**：Mesh Ping 定时探测（30 秒间隔）

### 10.8 代理配置上报

节点在入站 CRUD 操作后自动向 Hub 上报代理配置：

- 上报内容包括入站类型、标签、监听端口、地址、协议选项和 TLS 配置
- Hub 存储配置后用于订阅链接生成
- 手动同步也可触发全量上报
