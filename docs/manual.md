...
`Cluster Center` 面向多节点域运维。Hub 负责域成员、域版本、域资源只读视图和订阅聚合；B-UI 节点负责本地资源落地、节点直连通信、Mesh Ping 和面板更新执行。

Hub 不转发节点间消息。成员变更、域资源变更或更新通知发生后，节点会根据 Hub 快照中的 `base_url`、`endpoint_path`、`peer_token` 直接访问对端。

### 10.1 入口与前提

- 面板侧栏导航中的 `Cluster Center`。
- 当前节点可以访问目标 `b-cluster-hub`。
- 当前节点的 `Base URL` 可被其他域成员访问。
- 目标 Hub 域存在可用的 `domain_token` 或 Join URI。
- 如果使用 HTTPS，对外证书、反向代理和 Base URI 需要先确认正确。

### 10.2 注册新域

点击 `Register` 打开注册对话框。推荐使用 Hub 管理页提供的 Join URI：

```text
buihub://hub.example.com/domain?id=example.com&domain_token=xxxx&hub_protocol=https
```

粘贴到 `Join URI` 后，面板会自动解析：

- `Domain`：来自 `id` 查询参数，也是 Hub API 中的 `domain_id`。
- `Hub URL`：来自 URI 主机和 `hub_protocol`。
- `Token`：来自 `domain_token`。
- `Base URL`：当前节点对外访问地址，默认从浏览器当前地址推导。

如果没有 Join URI，也可以手动填写 `Domain`、`Hub URL`、`Token` 和 `Base URL`。提交后面板会轮询操作状态，注册完成后自动刷新域列表和成员列表。

### 10.3 域列表、成员和快照

注册成功后，左侧显示已加入的域；右侧显示选中域的成员快照。

常用字段：

- `Node ID`：节点身份，传播、请求和存储都以它为唯一键。
- `Name` / `Display Name`：面向运维人员的展示名称，不作为主键。
- `Base URL`：节点间直连地址。
- `Version`：该节点上报的 B-UI 面板版本。
- `Status`：Hub 记录的在线/离线状态。

当节点在 Hub 快照中发现自己已不再是成员时，会清理本地域镜像状态。运维上应把 Hub 快照视为成员关系的权威来源。

### 10.4 成员管理和同步

- **查看成员**：选中域后自动显示成员列表。
- **手动同步**：点击 `Manual Sync` 强制拉取 Hub 快照，并触发本地资源状态上报。
- **删除成员**：从 Hub 移除目标成员。新版节点会执行本地域资源清理；如果目标节点无法触达，按界面提示决定是否强制删除。
- **离开域**：当前节点离开域前会先清理本地域用户、域入站、TLS 记录和包装元数据，再提交 Hub 删除。

常规情况下不需要频繁手动同步；它主要用于恢复网络抖动、旧节点升级或资源视图不一致后的状态。

### 10.5 域入站

域入站用于把同一组代理入口落地到多个节点。长期身份是稳定的 `group_id`，不是某个节点上的本地入站 id。

创建或更新域入站时，需要关注：

- `group_id`：域入站组身份，由面板维护。
- `tag_seed`：入站标签的主体片段。
- `prefix` / `suffix`：可选标签前后缀。
- `target_members`：目标节点列表。
- `target_tag`：每个目标节点最终使用的本地 tag。
- `inbound`：协议、监听、传输、复用等入站配置。
- `tls_template` / `tls`：可选 TLS 或 Reality 模板。

本地 tag 生成规则：

```text
[prefix]-[tag seed]-[node display name]-[suffix]
```

空片段会被省略，所以最短 tag 是 `[node display name]`。当 Hub 或协调节点提供 `target_tag` 时，接收节点必须优先使用它，避免不同节点自行拼接导致订阅显示不一致。

### 10.6 域用户

域用户用于把用户配置应用到域入站组，并生成域订阅。

使用建议：

- 绑定入站时优先绑定稳定的域入站 group id。
- Hub 订阅会通过 `domain_inbound_group_id` 找到各节点上报的域入站配置。
- 如果老版本仍使用 `domain:<request_id>` 或原始 request id，Hub 会尽量兼容，但长期应迁移到 group id。
- 用户的 UUID、password、auth 等协议密钥可以由各落地节点本地生成，Hub/domain 用户资源 id 不应被这些本地值替代。

这意味着同一个域用户在不同节点上可以拥有不同的本地协议凭据，但在 Hub、订阅和管理页面里仍然是同一个域用户。
...
- 域内节点上报的代理配置。
- 外部 direct link，例如第三方 `vless://`、`trojan://`、`ss://`。
- 外部 subscription link，例如远端 Base64 订阅。

面板上报给 Hub 前会去除由本地生成的重复链接，并保留外部链接。Hub 生成订阅时会去重并按格式输出：

- `raw` / `base64`：合并域内链接和外部 direct link。
- `clash`：把域内节点和外部代理放入代理列表及代理组。
- `singbox`：把域内节点和外部 outbounds 放入 selector/urltest。
- `json`：输出结构化代理链接数据。

如果域用户没有绑定任何入站，Hub 默认只选取域作用域的代理配置，不会把普通节点本地入站混入域用户订阅。

### 10.8 Mesh Ping 和节点详情

点击成员节点可进入节点详情页：

- 查看节点 ID、展示名称、Base URL、面板版本、状态。
- 查看可折叠延迟卡片。
- 按目标成员筛选延迟结果。
- 通过颜色快速识别正常、警告和超时。

Mesh Ping 是节点本地视角的可达性结果，不会写回 Hub 成员状态。单节点域不会调度 peer probe，因为没有其他成员可探测。
...
- `INBOUND`：入站和域入站相关操作。
- `OUTBOUND`：出站相关操作。
- `HUB`：注册、快照、资源上报、订阅等 Hub 通信。
- `CRON`：定时任务和后台同步。

日志使用内存环形缓冲区，最多保留 2048 条；重启面板后清空。默认显示最近 200 条，可按域名过滤。

### 10.10 面板更新编排

每个节点会独立检测最新 B-UI 版本。域的 `update_policy` 决定自动化程度：

- `auto`：检测到新版本后可声明更新目标、广播可用性并启动本地更新。
- `manual`：只显示更新可用性，不自动开始本地更新。

节点更新状态会通过 Hub member-status 和节点间事件同步。更新中的节点会锁定行级更新操作；恢复在线后会刷新 `panel_version`。

### 10.11 使用建议

- 一个节点可以加入多个 Hub 域，但应确保 Base URL 在每个域内都可达。
- 优先使用 `buihub://` Join URI，减少手动填写错误。
- 注册令牌和域 token 应按最小范围保存，不要写入公开文档。
- 域入站、域用户、外部链接变更后，先检查 Hub 资源视图，再验证订阅输出。
- 删除成员前先确认目标节点是否在线；强制删除只应在目标不可恢复或明确要丢弃本地残留资源时使用。
