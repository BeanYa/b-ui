...
- **节点间通信协议**：节点通过 Hub 下发的成员表获取对端端点后直连通信，支持事件队列、幂等去重、广播、多目标投递、链式传播和定时广播的协议模型。Hub 只负责域成员权威登记，不参与节点间消息转发。
...
- 集群协议：[`BeanYa/b-cluster-protocol`](https://github.com/BeanYa/b-cluster-protocol)
...
- Hub 协议：`protocol/hub/v1`，定义注册、删除、操作查询、版本查询和成员快照。
- Node 协议：`protocol/nodes/v1`，定义 peer message envelope、路由、投递确认、链式传播和定时广播。
