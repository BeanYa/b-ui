...
LOCAL_INSTALL_REL="scripts/release/install.sh"

SCRIPT_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
REPO_ROOT=$(cd "${SCRIPT_DIR}/../.." && pwd)
LOCAL_CENTRAL_INSTALL_SCRIPT="${REPO_ROOT}/${LOCAL_INSTALL_REL}"
LOCAL_WRAPPER_INSTALL_SCRIPT="${REPO_ROOT}/install.sh"
TARGET_VERSION=""
...
run_install_script() {
    if [[ -f "${LOCAL_CENTRAL_INSTALL_SCRIPT}" ]]; then
        bash "${LOCAL_CENTRAL_INSTALL_SCRIPT}" "$@"
        return
    fi

    if [[ -f "${LOCAL_WRAPPER_INSTALL_SCRIPT}" ]]; then
        bash "${LOCAL_WRAPPER_INSTALL_SCRIPT}" "$@"
        return
    fi
